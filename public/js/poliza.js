...
function verproducto(inp) {
	...
	$.getJSON(
		...
		function (data) {
			...
			$("#tipbulto option[value=" + data.tipo_bulto + "]").attr(
				"selected",
				true
			);
...
function add_adjunto(id, dua, num_item) {
	var formData;
	url_destino =
		"index.php/poliza/crear/guardar_adjunto/" + id + "/" + dua + "/" + num_item;
...
function get_item_adjunto() {
	$("#add_adjuntos").on("show.bs.modal", function (e) {
		...
		$(e.currentTarget).find('input[name="item_adjunto"]').val(bookId);
		$(e.currentTarget).find('input[name="dua_adjunto"]').val(bookId1);
...
function detalle_poliza() {
	...
	$.getJSON(
		...
		function (data) {
			...
			var url = base_url("index.php/poliza/crear/obtener_datos_file_gacela/" + data.c807_file);
			
		
			$.get(url, function (data) {
				let datos=JSON.parse(data);
				//alert(datos.NOMBRE);
				$("#nit_consignatario").val(datos.NIT);
				$("#nit_consignatario").blur();
				$("#incoterm").val(datos.incoterm);
				$("select").trigger("chosen:updated");
			});
		}
	);
...
function regimen(modelo, reg_e, reg_a) {
	//  alert(reg_e);
	var url = base_url(
		"index.php/poliza/crear/get_regimen/" + modelo + "/" + reg_e + "/" + reg_a
	);
...
function download_xml() {
	...
	var url = base_url(
		"index.php/poliza/crear/download_xml/" + "R" + referencia + ".xml"
	);
	$.ajax({
		method: "POST",
		url: url,
...
function generar_xml() {
	...
	referencia = $("#referencia").val();
	var url = base_url(
		"index.php/poliza/crear/generar_xml/" + dua + "/" + referencia
	);
	$.ajax({
		method: "POST",
		url: url,
