...
class Subir_Archivo extends CI_Controller
{
            ...
            if (!$id_file) {
                ...
                foreach ($object->getWorksheetIterator() as $worksheet) {
                    ...
                    for ($row = 2; $row <= $highestRow; $row++) {
                        ...
                        $origen_incorrecto     = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
                        $origen_incorrecto = strlen(trim($origen_incorrecto));
                        if ($origen_incorrecto > 2) {
                            $no_procesar = 1;
                        }
                    }
                }
                ...
                if ($no_procesar == 1) {
                    $_SESSION["no_clasificado"] = 'Se han encontrado inconsistencias en el pais de origen, favor corregir';
                    redirect("subir_archivo");
                }
                    ...
                    foreach ($object->getWorksheetIterator() as $worksheet) {
                        ...
                        for ($row = 2; $row <= $highestRow; $row++) {
                            ...
                            if (is_null($codigo) || empty($codigo) || $codigo == "") {
                                ...
                                $numero_partida = "";
                                $datos = $this->Subir_archivos_model->verificar_partida($codigo, $nit_importador, $_SESSION['pais_id'], $pais_origen);
    ...
    {
        if (
            isset($_POST["partida_arancelaria" . $id_reg]) && strlen(trim($_POST["partida_arancelaria" . $id_reg])) > 0
    ...
    {
        if (
            isset($_GET["c807_file"]) &&  isset($_GET["doc_transporte"]) && isset($_GET["tot_bultos"]) && isset($_GET["tot_kilos"])
